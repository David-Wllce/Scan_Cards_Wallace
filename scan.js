    ...
    for (const contact of contacts) {
      try {
        if (contact.email && existingEmails.has(contact.email.toLowerCase())) {
          results.skipped.push({ nom: [contact.prenom, contact.nom].filter(Boolean).join(" "), reason: "Email déjà existant" });
          continue;
        }
        ...
        if (page.object === "error") {
          ...
          results.created.push({ nom: nomComplet, url: page.url });
          if (contact.email) existingEmails.add(contact.email.toLowerCase());
        }
        await new Promise(r => setTimeout(r, 120));
      } catch (err) {
        results.errors.push({ nom: [contact.prenom, contact.nom].filter(Boolean).join(" "), reason: err.message });
      }
    }
